...
def price_above_avg(price_total: float, prod_price_avg: float) -> bool:
    ...
    - prod_price_avg (from agg_results)
...
print("  - prod_quantity_sum (Case 3)")
print("  - prod_price_sum (Case 3)")
print("  - prod_price_avg (Case 3) <-- Used by price_above_avg filter!")
print("  - prod_price_gt_avg_count (Case 3)")


# ### Store Features

# Initialize feature store with fidx_config
feature_store = FeatureStore(base_cfg.get('fidx_config'))
...
# Store features with optional auto-save to filesystem
# Set auto_save=True to persist features to disk
feature_store.store_features(
    features,
    model_name='product_stats',
    auto_save=False  # Set to True to save features to filesystem
)
...
print("\nNotice: prod_price_avg comes BEFORE price_above_avg in execution order!")
...
# The `price_above_avg` filter successfully uses `prod_price_avg` attribute as input, demonstrating the KEY INNOVATION of the 4-case logic.
# 
# ### 📚 Next Steps
